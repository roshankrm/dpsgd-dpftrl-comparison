...
# Privacy parameters 
NOISE_MULTIPLIER = 0.635
MAX_GRAD_NORM = 1.1
DELTA = 1e-5  # Target delta

# Create directory for saving plots
PLOTS_DIR = "dp_lenet_plots"
...
def plot_and_save_results(train_losses, train_accs, val_losses, val_accs, epsilons):
    ...
    # Create a more detailed filename with parameters
    filename_base = f"lenet_noise_{NOISE_MULTIPLIER}_gradnorm_{MAX_GRAD_NORM}"
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title(f'Loss Curve (σ={NOISE_MULTIPLIER}, C={MAX_GRAD_NORM})')
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.title(f'Accuracy Curves (σ={NOISE_MULTIPLIER}, C={MAX_GRAD_NORM})')
    
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, f"{filename_base}_noise_{NOISE_MULTIPLIER}_results.png"))
    plt.close()
    ...
    plt.title(f'Privacy Budget Growth (σ={NOISE_MULTIPLIER}, C={MAX_GRAD_NORM})')
    plt.grid(True)
    plt.savefig(os.path.join(PLOTS_DIR, f"{filename_base}_noise_{NOISE_MULTIPLIER}_privacy_budget.png"))
    plt.close()

def plot_confusion_matrix(all_labels, all_preds):
    """Plot and save confusion matrix"""
    filename = f"lenet_noise_{NOISE_MULTIPLIER}_gradnorm_{MAX_GRAD_NORM}_confusion_matrix.png"
    ...
    plt.title(f'Confusion Matrix (σ={NOISE_MULTIPLIER}, C={MAX_GRAD_NORM})')
...
def main():
    ...
    # Save final model
    torch.save(model.state_dict(), os.path.join(PLOTS_DIR, f"lenet_noise_{NOISE_MULTIPLIER}_gradnorm_{MAX_GRAD_NORM}.pt"))
