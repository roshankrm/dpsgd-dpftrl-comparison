...
# Hyperparameters
BATCH_SIZE = 500  # Lot size in the paper's terminology
TEST_BATCH_SIZE = 500
N_EPOCHS = 20
LEARNING_RATE = 0.1 # Initial learning rate
MOMENTUM = 0.9
LOG_INTERVAL = 10
MAX_PHYSICAL_BATCH_SIZE = 500  # For memory management

# Privacy parameters 
TARGET_EPSILON = 23.80  # Target privacy budget
MAX_GRAD_NORM = 1.1
DELTA = 1e-5  # Target delta

# Create directory for saving plots
PLOTS_DIR = "dp_lenet_plots_fixed_epsilon"
...
def plot_and_save_results(train_losses, train_accs, val_losses, val_accs, epsilons):
    """Plot and save results"""
    filename_base = f"lenet_epsilon_{TARGET_EPSILON}_gradnorm_{MAX_GRAD_NORM}"
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title(f'Loss Curve (ε={TARGET_EPSILON}, C={MAX_GRAD_NORM})')
    
    # Accuracy curves subplot
    plt.subplot(1, 2, 2)
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.title(f'Accuracy Curves (ε={TARGET_EPSILON}, C={MAX_GRAD_NORM})')
    
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, f"{filename_base}_results.png"))
    plt.close()
    
    # Plot epsilon growth over epochs
    plt.figure(figsize=(8, 5))
    plt.plot(range(1, len(epsilons) + 1), epsilons, marker='o')
    plt.xlabel('Epochs')
    plt.ylabel('Privacy Budget (ε)')
    plt.title(f'Privacy Budget Growth (ε={TARGET_EPSILON}, C={MAX_GRAD_NORM})')
    plt.grid(True)
    plt.savefig(os.path.join(PLOTS_DIR, f"{filename_base}_privacy_budget.png"))
    plt.close()

def plot_confusion_matrix(all_labels, all_preds):
    """Plot and save confusion matrix"""
    filename = f"lenet_epsilon_{TARGET_EPSILON}_gradnorm_{MAX_GRAD_NORM}_confusion_matrix.png"
    ...
    plt.title(f'Confusion Matrix (ε={TARGET_EPSILON}, C={MAX_GRAD_NORM})')
...
def main():
    ...
    # Make model, optimizer and data loader private using make_private_with_epsilon
    ...
    # Print the calculated noise multiplier
    ...
    # Final test to get confusion matrix data
    ...
    # Plot and save results
    plot_and_save_results(train_losses, train_accs, val_losses, val_accs, epsilons)
    ...
    # Save final model
    torch.save(model.state_dict(), os.path.join(PLOTS_DIR, f"lenet_epsilon_{TARGET_EPSILON}_gradnorm_{MAX_GRAD_NORM}.pt"))
